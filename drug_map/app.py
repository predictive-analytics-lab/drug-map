...
base_ui = html.Div(
    ...
    children=[
        ...
        dcc.RadioItems(
            id="smoothingparameter", value="1.0", style=dict(display="none")
        ),
        ...
        html.Div(
            html.Label(
                [
                    ...
                    dcc.Dropdown(
                        id="drugtype",
                        options=[{"label": "Cannabis", "value": "cannabis"}],
                        style={"width": "100%", "display": "block"},
                        value="cannabis",
                    ),
                ]
                ...
                [
                    ...
                    dcc.RadioItems(
                        ...
                        options=[
                            ...
                            {
                                "label": "Age, Race, Sex, Poverty Status",
                                "value": "poverty",
                            },
...
smoothing_ui = html.Div(
    ...
    children=[
        ...
        dcc.RadioItems(id="usagetarget", value="using", style=dict(display="none")),
        html.H2("USA DRUG BIAS MAP"),
        html.P(
            """Select different drugs, as well as different map types and usage models by changing the options below."""
        ),
...
@app.callback(
    [
        Output("ui-div", "children"),
        Output("standard_tab", "children"),
        Output("smoothed_tab", "children"),
    ],
    [Input("tabs", "value")],
)
...
def update_data(
    ...
    republican: List[str],
    urban_filter: int,
    smoothing_parameter: str,
    usage_target: str,
    tab: str,
) -> dict:
    df = mapping.args_to_df(
        drug_type=drugtype,
        smoothed=tab == "smoothed",
        year=time,
        citype=citype,
        model=model,
        republican_cats=republican,
        urban_filter=str(urban_filter),
        smoothing_param=float(smoothing_parameter),
...
app.clientside_callback(
    ClientsideFunction(namespace="clientside", function_name="get_map"),
