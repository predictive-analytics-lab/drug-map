...
@app.callback(Output('drug-map','figure'),[Input('drugtype','value'),Input('maptype','value')],[State('drug-map', 'relayoutData')])
@cache.memoize(timeout=timeout)
def update_graph(drugtype: str, maptype: str, relayout_data: dict) -> plotly.graph_objs.Figure:
    fig = args_to_map(drug_type=drugtype, smoothed=False, map_type=maptype)
    if relayout_data:
